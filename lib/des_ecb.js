/**
 * DES de un bloque (ECB, 8 bytes) — implementación educativa mínima.
 * Evita dependencia de OpenSSL legacy en Node moderno.
 */
...
function desBlock(block8, key8, decrypt) {
  if (block8.length !== 8 || key8.length !== 8) {
    throw new Error("DES educativo requiere bloques y claves de 8 bytes");
  }
...
function desEncryptBlock(key8, block8) {
  return desBlock(block8, key8, false);
}

function desDecryptBlock(key8, block8) {
  return desBlock(block8, key8, true);
}

module.exports = { desEncryptBlock, desDecryptBlock };